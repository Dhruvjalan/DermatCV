...
# ── PYDANTIC SCHEMAS (With tunable hyperparams) ─────────────────────────
...
class UpdateGraphRequest(BaseModel):
    data: List[Dict[str, str]] # e.g., [{"disease": "Acne", "text": "pimples, oily skin"}]

# ── CORE SESSION LOGIC ──────────────────────────────────────────────────

class DiagnosticSession:
    def __init__(self, session_id: str, ml_predictions: list, hyperparams: dict):
    ...
    def load(session_id: str):
        ...
        with open(path) as f:
            return DiagnosticSession.from_dict(json.load(f))

# Helper Functions (Abstracted from your Notebook)
...
def compute_scores(candidate_diseases, confirmed, denied, ml_preds, hp: dict):
    ...
    for disease in candidate_diseases:
        ...
        results.append({
            "disease": disease,
            "score": round(max(raw_score / total_syms, 0.0), 5),
            "prior": round(prior, 4)
        })
...
async def start_diagnosis(req: InitialDiagnosticRequest):
    """Parses initial symptoms and returns the first question to the frontend."""
    session_id = datetime.utcnow().strftime("%Y%m%d_%H%M%S")
    raw_syms = [s.strip() for s in re.split(r"[,;]+", req.symptom_text) if len(s.strip()) > 2]
    
    # Simulate ML Prior based on initial input
    ml_predictions = [{"disease": d, "confidence": 0.5} for d in raw_syms] # Simplified for API
    ...
    if not symptom_to_ask:
        raise HTTPException(status_code=400, detail="Cannot generate discriminating question from input.")
...
async def process_answer(req: AnswerRequest):
    """Processes an answer, checks confidence dropouts, and returns next step or final report."""
    try:
        session = DiagnosticSession.load(req.session_id)
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail="Session not found")

    # 1. Parse previous answer
...
@app.post("/api/graph/update", tags=["Reinforcement Learning"])
async def update_graph_data(req: UpdateGraphRequest):
    """Endpoint to pass new datasets to iteratively update graph weights."""
    # Logic to batch extract and nx.write_graphml(G) goes here
    # Similar to `incremental_update` from notebook
    return {"message": f"Graph successfully updated with {len(req.data)} new entries."}

@app.get("/api/graph/visualize/{type}", tags=["Visualizations"])
async def get_visualization(type: str):
    """Fetches the generated visual files. Acceptable types: 'interactive' or 'static'"""
    if type == "interactive":
        if os.path.exists(GRAPH_HTML_PATH):
            return HTMLResponse(content=open(GRAPH_HTML_PATH, "r").read())
        raise HTTPException(status_code=404, detail="HTML Graph not found")
        
    elif type == "static":
        if os.path.exists(GRAPH_STATIC_PATH):
            return FileResponse(GRAPH_STATIC_PATH, media_type="image/png")
        raise HTTPException(status_code=404, detail="Static Graph Image not found")
        
    raise HTTPException(status_code=400, detail="Type must be 'interactive' or 'static'")
...
async def download_report(filename: str):
    """Serves the generated session reports to the frontend."""
    # Ensure secure path resolution to prevent directory traversal
    ...
    if os.path.exists(file_path):
        return FileResponse(
            ...
            media_type='text/plain' # Or 'application/pdf' if you implement PDF generation later
        )
